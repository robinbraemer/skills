...
---
## When to Use
Use when deciding where an AI task should run and quota, provider limits, model quality, account pools, or reset windows may affect the choice. Applies to automatic routers and human-facing recommendations. Do not use for merely reporting quota without making or explaining a selection.
...
5. Use reported reset times, freshness, provider status, and per-window limits. Prefer live/fresh quota data; be conservative with stale or unknown data.
