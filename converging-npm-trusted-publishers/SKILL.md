...
`--verify-only` (no `--ledger`) is a strict read-only status survey: it opens every manifest package's tab, reports the publisher tuple npm currently shows for each — `existing owner/repo workflow`, `absent`, `unsaved-form`, or `unreadable (reason)` — and exits after that report. It never stages, types, clicks Edit/Save/Delete, or writes a ledger file, and unlike the normal run it does **not** stop at the first blocked/drifted package: every package gets its own report line so one bad package never hides the status of the rest of the fleet. Use it to check progress mid-migration or resolve whether a save actually landed before deciding whether to resume the automated path or finish the rest by hand.
